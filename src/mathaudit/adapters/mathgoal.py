...
class MathGoalAdapter:
    ...
    def can_handle(self, payload: Dict[str, Any]) -> bool:
        compact = {"id", "pred", "llm_calls"}.issubset(payload) or {
            "idx",
            "prediction",
            "api_calls",
        }.issubset(payload)
    ...
    def _convert_compact(
        self, payload: Dict[str, Any], problem: ProblemContext, run: RunContext
    ) -> Episode:
        base = base_episode_parts(
            ...
            warnings=[
                "Compact MathGoal output exposes only the final prediction and aggregate cost."
            ],
        )
        source = Source(
        ...
        observation = SourceObservation(
            ...
            outcome=outcome,
            cost=Cost(
                calls=self._int_or_none(
                    payload.get("llm_calls")
                    if payload.get("llm_calls") is not None
                    else payload.get("api_calls")
                ),
    ...
    def _convert_full(
            ...
            if raw_role != "tool_corrected":
                ...
                sources.append(
                    Source(
                        ...
                        producer="MathGoal",
                        tool="python/sympy",
                        provenance_group="mathgoal_candidate:%s" % candidate_key,
                        metadata={"candidate_id": candidate_key, "candidate_role": raw_role},
                    )
                )
                tool_observation = "observation:%s" % tool_source
                if isinstance(tool_report, dict):
                    tool_text = str(
                        tool_report.get("extracted_answer")
                        or tool_report.get("stdout")
                        or tool_report.get("stderr")
                        or ""
                    ).strip()
                    ...
                    observations.append(
                        SourceObservation(
                            observation_id=tool_observation,
                            source_id=tool_source,
                            stage="tool",
                            sequence=sequence,
                            opportunity=Opportunity.eligible,
                            ...
                            cost=Cost(
                                latency_s=self._float_or_none(tool_report.get("elapsed_seconds")),
                                tool_executions=1,
                            ),
                            metadata={
                                key: tool_report.get(key)
                                for key in (
                                    "blocked",
                                    "timed_out",
                                    "independent_work",
                    ...
                    if tool_outcome == ObservationOutcome.produced:
                        ...
                        evidence.append(
                            Evidence(
                                ...
                                stage="tool",
                                sequence=sequence,
                                content=make_content(tool_text),
                                metadata={"evidence_flags": tool_report.get("evidence_flags", [])},
                            )
                        )
                        if candidate_key in candidate_ids:
                            edges.append(
                                ProvenanceEdge(
                                    from_id=candidate_ids[candidate_key],
                                    to_id=tool_id,
                    ...
                    observations.append(
                        SourceObservation(
                            ...
                            invocation=Invocation.not_called,
        ...
        for corrected_key, original_key in corrected_candidates:
            ...
            if original_key in candidate_ids:
                edges.append(
                    ProvenanceEdge(
                        from_id=candidate_ids[original_key],
                        to_id=corrected_id,
                        relation=EdgeRelation.derived_from,
                        observability=EdgeObservability.adapter_inferred,
                    )
            ...
            if original_key in tool_evidence_ids:
                edges.append(
                    ProvenanceEdge(
                        from_id=tool_evidence_ids[original_key],
                        to_id=corrected_id,
                        relation=EdgeRelation.derived_from,
                        observability=EdgeObservability.adapter_inferred,
                    )
        ...
        verifier_source = Source(
            source_id="mathgoal_verifier",
            source_type=SourceType.llm,
            role="verifier",
            producer="MathGoal",
            provenance_group="mathgoal_verifier",
        )
        ...
        if not verification_reports:
            observations.append(
                SourceObservation(
                    observation_id="observation:verifier:0",
        ...
        if final_text:
            ...
            decisions.append(
                Decision(
                    ...
                    candidate_evidence_ids=list(candidate_ids.values()),
                    selected_evidence_ids=[],
        ...
        return Episode(
            ...
            final_output=FinalOutput(
                status=FinalStatus.produced if final_id else FinalStatus.empty, evidence_id=final_id
            ),
            audit_only=base[4],
            metadata={
                "classification": payload.get("classification"),
                "plan": payload.get("plan"),
                "mathgoal_metadata": payload.get("metadata"),
            },
        )
